...
public class PluginLoader implements PluginLoaderIF {
	...
	public PluginLoader() {
		super();
		this.classLoader = SpringApplication.class.getClassLoader();
	}
	...
	public List<Class<?>> loadAllPlugins(FilterIF filter) throws PluginException {
		...
		for (String element : classPathElements) {
			...
			for (String aResource : aResources) {
				if (aResource.contains("org.openmarkov")) {
					try {
						Class<?> aClass = (Class<?>) classLoader.loadClass(aResource.replace("BOOT-INF.classes.", ""));
